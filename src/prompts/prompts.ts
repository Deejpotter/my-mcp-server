...
Remember: Thorough planning prevents integration issues. Research first, implement carefully, test thoroughly!`,
	...
	// PROMPT 10: GROCY RECIPE WORKFLOW
	server.prompt(
		"grocy_recipe_workflow",
		"Complete workflow for creating recipes in Grocy with ingredients, products, and fulfillment tracking",
		{
			recipe_name: z
				.string()
				.optional()
				.describe("Name of the recipe to create (e.g., 'Spaghetti Bolognese')"),
			recipe_type: z
				.string()
				.optional()
				.describe(
					"Type of recipe: 'breakfast', 'lunch', 'dinner', 'snack', 'dessert'"
				),
		},
		// eslint-disable-next-line @typescript-eslint/require-await
		async (args) => {
			const recipeName = args?.recipe_name || "[your recipe name]";
			const recipeType = args?.recipe_type || "meal";
				...
				messages: [
					{
						...
						content: {
							type: "text" as const,
							text: `You are a Grocy kitchen management expert who helps users create and manage recipes in their Grocy instance. You guide users through finding or creating recipe ingredients, building complete recipes with proper quantities, and tracking recipe fulfillment.`,
						},
					},
					{
						role: "user" as const,
						content: {
							type: "text" as const,
							text: `# Grocy Recipe Creation Workflow
...
## Overview
This workflow helps you create a complete recipe in Grocy, from researching ingredients to tracking fulfillment. Grocy requires all ingredients to be existing products in your database before you can build a recipe.

## Phase 1: Research & Recipe Finding

### 1.1 Find a Recipe (If Needed)
If you don't have a recipe yet, search online:

**Use DuckDuckGo or Google Search:**
\`\`\`
duckduckgo_search: "${recipeName} recipe simple ingredients"
google_search: "easy ${recipeName} recipe step by step"
\`\`\`

**What to look for:**
- Simple ingredient list (5-15 items)
- Clear quantities (cups, grams, pieces)
- Basic cooking instructions
- Realistic serving sizes

**Example Search Results:**
- BBC Food recipes (reliable measurements)
- AllRecipes (user-tested recipes)
- Serious Eats (detailed instructions)

### 1.2 Understand Recipe Structure in Grocy
**Grocy Recipe Components:**
1. **Recipe Object**: Name, description, servings
2. **Recipe Positions (ingredients)**: Product ID + quantity + quantity unit
3. **Recipe Fulfillment**: Check if you have all ingredients in stock

**Important Concepts:**
- **Base Servings**: How many servings the recipe makes
- **Desired Servings**: How many you want to make (scales quantities)
- **Product IDs**: Must reference existing products in Grocy
- **Quantity Units**: Must match product's stock quantity unit

## Phase 2: Check Grocy Product Availability

### 2.1 List Current Stock
...
### 2.2 Search for Recipe Ingredients
**For each recipe ingredient, check if product exists:**
\`\`\`typescript
// Use grep on the stock response or search products list
// Look for ingredient names that match recipe ingredients
\`\`\`

**Example Mapping:**
\`\`\`
Recipe Ingredient → Grocy Product
"Ground Beef" → Find product_id for "Ground Beef" or "Minced Beef"
"Spaghetti" → Find product_id for "Pasta" or "Spaghetti"
"Tomato Sauce" → Find product_id for "Tomato Sauce" or "Passata"
"Onion" → Find product_id for "Onions"
"Garlic" → Find product_id for "Garlic"
\`\`\`

### 2.3 Create Missing Products (If Needed)
**If products don't exist, you need to create them first:**

This workflow focuses on using existing products. To create new products:
1. Use Grocy web UI to add products
2. Define product name, quantity unit, location
3. Get the product ID for use in recipe

**Note:** Product creation via API requires using the generic \`objects/products\` endpoint, which is not currently implemented in the MCP tools.

## Phase 3: Create the Recipe

### 3.1 Create Base Recipe
**Use the generic objects endpoint:**
\`\`\`
POST /objects/recipes
{
  "name": "${recipeName}",
  "description": "Simple ${recipeType} recipe",
  "base_servings": 4,
  "desired_servings": 4
}
\`\`\`

**This creates the recipe and returns:**
\`\`\`json
{
...
**Save this recipe_id** - you'll need it for adding ingredients!

### 3.2 Add Ingredients to Recipe
**For each ingredient, create a recipe position:**
\`\`\`
POST /objects/recipes_pos
{
  "recipe_id": 123,
  "product_id": [product_id from your research],
  "amount": [quantity from recipe],
...
\`\`\`json
// Ingredient 1: Ground Beef (500g)
{
  "recipe_id": 123,
  "product_id": 45,  // ID for "Ground Beef" product
  "amount": 500,
  "qu_id": 2,        // ID for "grams" unit
  "note": "Lean ground beef works best"
}

// Ingredient 2: Spaghetti (400g)
{
  "recipe_id": 123,
  "product_id": 78,  // ID for "Spaghetti" product
  "amount": 400,
  "qu_id": 2,        // ID for "grams" unit
}

// Ingredient 3: Tomato Sauce (800ml)
{
  "recipe_id": 123,
  "product_id": 12,  // ID for "Tomato Sauce" product
  "amount": 800,
  "qu_id": 5,        // ID for "ml" unit
}
\`\`\`

**How to find quantity_unit IDs:**
- Use \`grocy_stock_get_product\` to see product's qu_id_stock
- Common units: grams, pieces, liters, ml
- Units are defined in Grocy's quantity_units table

## Phase 4: Test Recipe Fulfillment

### 4.1 Check if Recipe is Fulfillable
**Test if you have all ingredients in stock:**
\`\`\`typescript
...
**Response tells you:**
- \`need_fulfilled\`: true/false - can you make it now?
- \`need_fulfilled_with_shopping_list\`: true/false - can you make it after shopping?
- \`missing_products_count\`: how many ingredients you're missing
- \`costs\`: estimated cost based on product prices

### 4.2 Add Missing Ingredients to Shopping List
**If recipe isn't fulfilled:**
\`\`\`typescript
grocy_recipe_add_missing_to_shoppinglist({
  recipe_id: 123,
  excluded_product_ids: [5, 12]  // Optional: products to skip
})
\`\`\`

**This automatically adds:**
- All missing products to your shopping list
- Correct quantities needed
- Considers what you already have in stock

### 4.3 When Ready to Cook
**After shopping, consume ingredients:**
\`\`\`typescript
...
**This will:**
- Remove correct quantities from stock
- Update product amounts
- Track recipe usage
- Only works if recipe is fully fulfilled

## Phase 5: Document in BookStack

### 5.1 Search for Existing Documentation
\`\`\`typescript
bookstack_search({
  query: "Grocy recipes"
})
...
### 5.2 Create or Update Recipe Documentation
**Create a new page:**
\`\`\`typescript
bookstack_create_page({
  book_id: 6,  // MCP Server book or relevant book
  name: "${recipeName} - Grocy Recipe",
  markdown: \`# ${recipeName}

## Overview
Simple ${recipeType} recipe for ${recipeName}.

## Grocy Recipe Details
- **Recipe ID**: 123
- **Base Servings**: 4
- **Created**: \${new Date().toISOString()}

## Ingredients
- Ground Beef: 500g (Product ID: 45)
- Spaghetti: 400g (Product ID: 78)
- Tomato Sauce: 800ml (Product ID: 12)
- Onion: 1 piece (Product ID: 23)
- Garlic: 3 cloves (Product ID: 34)

## Fulfillment Status
Check with:
\\\`\\\`\\\`
grocy_recipe_get_fulfillment({ recipe_id: 123 })
\\\`\\\`\\\`

## Cooking Instructions
1. [Add cooking steps from recipe source]
2. [Include timing and temperatures]
3. [Note serving suggestions]

## Notes
- Original recipe source: [URL]
- Created date: \${new Date().toLocaleDateString()}
- Last cooked: [Track when you make it]
\`
})
\`\`\`

## Complete Example: Spaghetti Bolognese

### Step 1: Research Recipe
\`\`\`typescript
duckduckgo_search({
  query: "spaghetti bolognese recipe simple"
})
\`\`\`
...
### Step 2: Check Grocy Products
\`\`\`typescript
grocy_stock_get_current()
\`\`\`

**Map ingredients to product IDs:**
- Ground Beef → product_id: 45
- Spaghetti → product_id: 78
- Tomato Sauce → product_id: 12
- Onions → product_id: 23
- Garlic → product_id: 34

### Step 3: Create Recipe
**Note:** This requires using the generic objects endpoint, which isn't directly implemented as a tool yet. The implementation requires:
...
// Returns: { "created_object_id": 123 }

// 2. Add each ingredient
POST /objects/recipes_pos (5 times, one for each ingredient)
\`\`\`
...
**If not fulfilled:**
\`\`\`typescript
grocy_recipe_add_missing_to_shoppinglist({ recipe_id: 123 })
\`\`\`

### Step 5: Cook and Track
**When ready:**
\`\`\`typescript
grocy_recipe_consume({ recipe_id: 123 })
\`\`\`

## Tool Reference

### Grocy Tools Available:
- \`grocy_system_info\` - Check Grocy connection
- \`grocy_stock_get_current\` - List all products in stock
- \`grocy_stock_get_product\` - Get specific product details
- \`grocy_stock_get_volatile\` - Check expiring/missing products
- \`grocy_recipe_get_fulfillment\` - Check if recipe is makeable
- \`grocy_recipe_consume\` - Consume recipe ingredients
- \`grocy_recipe_add_missing_to_shoppinglist\` - Add missing to list
- \`grocy_shoppinglist_add_product\` - Manually add to shopping list
- \`grocy_shoppinglist_add_missing\` - Add all missing products

### Not Yet Implemented (need generic object tools):
- Recipe creation (POST /objects/recipes)
- Recipe ingredient addition (POST /objects/recipes_pos)
- Product creation (POST /objects/products)

**Workaround:** Use Grocy web UI to create recipes, then use MCP tools for fulfillment tracking and shopping list management.

## Common Issues & Solutions

### Issue: Product not found in Grocy
**Solution:** Create product in Grocy web UI first:
1. Go to Master Data > Products
2. Add product with name, quantity unit, location
3. Note the product_id for recipe creation

### Issue: Wrong quantity units
**Solution:** Check product's stock quantity unit:
\`\`\`typescript
grocy_stock_get_product({ product_id: 45 })
// Returns qu_id_stock: 2 (use this for recipe)
\`\`\`

### Issue: Recipe not fulfilled
**Solution:** Check what's missing:
\`\`\`typescript
grocy_recipe_get_fulfillment({ recipe_id: 123 })
// Shows missing_products_count and details
\`\`\`
...
### Issue: Can't consume recipe
**Solution:** Recipe must be 100% fulfilled. Check:
1. All ingredients have sufficient stock
2. Amounts are in correct units
3. Products aren't expired or spoiled

## Best Practices

### Recipe Planning:
✅ Use common, simple ingredients
✅ Check stock before planning meals
✅ Build shopping list from unfulfilled recipes
✅ Track costs with Grocy price data

### Product Management:
✅ Keep product names consistent
✅ Use standard quantity units
✅ Set minimum stock levels
✅ Update stock after shopping

### Workflow Efficiency:
✅ Create recipes for frequent meals
✅ Use recipe fulfillment before shopping
✅ Consume recipes after cooking (tracks usage)
✅ Document favorite recipes in BookStack

### Documentation:
✅ Link BookStack pages to recipe IDs
✅ Include original recipe sources
✅ Note modifications and preferences
✅ Track cooking times and results

## Next Steps

### To create your first recipe:
1. **Find recipe**: Search online for ${recipeName}
2. **Check products**: List current stock, map ingredients
3. **Create recipe**: Use Grocy web UI (POST /objects/recipes)
4. **Add ingredients**: Use web UI (POST /objects/recipes_pos)
5. **Test fulfillment**: Use grocy_recipe_get_fulfillment
6. **Shop**: Add missing to shopping list
7. **Cook**: Consume recipe when ready
8. **Document**: Create BookStack page

### To extend functionality:
- Implement generic object creation tools
- Add recipe search and listing
- Create meal planning workflows
- Build shopping optimization tools

Remember: Grocy recipes help you plan meals, track ingredient usage, and manage your kitchen efficiently!`,
						},
					},
				],
			};
